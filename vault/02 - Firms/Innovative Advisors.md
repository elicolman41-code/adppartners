---
type: firm
status: pipeline
---
# Innovative Advisors — Stephen

**Standing:** LIVE — moving. Call went great Fri 2026-09-04. **Next meeting WEDNESDAY.**

## Where it stands
Full process walked end to end. He's in. Remaining path: **fill the Excel → get it approved → get it signed off → move.**

## THE BIG UPDATE
**2x multiplier CONFIRMED through end of October** (was Sept 30). His reaction: "okay, that's great." Pressure is off — but that means the date now has to come from him, not from a deadline.

## What he asked, and what I told him

**"Paychex is charging me ~$60/month per employee."**
Eli pushed back on the call. **CORRECTION 2026-09-17 — STEPHEN WAS RIGHT.** His own sheet: $18,186.82 Paychex book ÷ 12 ÷ 25 employees = **$60.62/employee/month** (all 18 clients: $60.34). Do NOT push back on this number again. It is an AVERAGE, not a rate card — per-client it ranges from $25 (Spencer Thwaytes, Broadway 2000) to $258 (Kranz, weekly, 1 EE). It STRENGTHENS the Grid case: it's the cost he's paying to do work he doesn't want.

**Billing access —** he's a **SUPER ADMIN** on the Paychex side. That's the unlock: he should have access to all the reports without needing per-client permission grants. This was the blocker for three weeks and it may not be a blocker at all.

**"How much of this is automatic?"**
All of it. Reports, filings — can be set up to run automatically.

**"Who's my point of contact?"**
Me, for everything. Onboarding, and internal issues too — notices, problems, all of it. He doesn't chase internal teams; I deal with them from there. This mattered to him.

**"How organized is the onboarding?"**
I have a full layout of exactly what clients need. It's simple. (See below.)

## WHAT I NEED FROM THE CLIENTS
**Bank proof only** — one of:
- recent bank statement
- voided check
- online banking screenshot

That's it. Keep the ask this small when it goes to him in writing.

## NOT COVERED — do Wednesday
**401(k) never came up.** Still needs a full walkthrough:
- 12 months free promo
- $500 per 401(k)
- 2x multiplier on payroll annual revenue
- licensed retirement partner to join

## Next steps
- [ ] **WEDNESDAY meeting** — set
- [ ] Walk 401(k) fully
- [ ] Chase the billing now that super-admin access is confirmed
- [ ] Verify the $60/employee/month figure against real invoices
- [ ] Excel → approval → sign-off

**Program fit:** [[Grid]] (2x, thru end of October) · 401(k) · [[Revenue Share]]


## Meeting 2026-09-10, 1:00 PM — plan
1. **McKenzie (licensed retirement partner) joins** — start the 401(k) process, give Stephen instructions on how to pull invoicing for the 401(k)s so we can get them
2. **Go through the Excel** — see what he has, answer questions
3. **Offer: Eli logs into his Paychex admin page** if he wants help pulling everything

## ⚠️ On the Paychex admin access
Do NOT take his credentials. **Screen share instead** — he drives, Eli directs. Reasons:
- Taking a client's login to a third-party system is a liability Eli does not want to own
- Paychex terms generally prohibit credential sharing; if anything looks wrong later, the access becomes the story
- Screen share gets the same result in the same time
If he insists on direct access, have him create a separate user rather than hand over his own.

## Status
- 2x multiplier runs through END OF OCTOBER
- He is a Paychex SUPER ADMIN — billing access should not be blocked
- Still to verify: the ~$60/employee/month claim, against real invoices


## ⭐ THE MOST IMPORTANT THING — keep the client ask tiny
**BANK PROOF is really the only thing we need from his clients.**
Accepted: recent bank statement · voided check · online banking screenshot.
Keep this as the single ask in anything written. A one-line ask gets done; a list gets deferred.

## Timing
- Implementation call-in happens around the **60 and 90 day** marks
- 401(k) plan transfers from Paychex also run **60-90 days** — argues for starting NOW, not January

## ⚠️ TO CONFIRM WITH ELI
Eli's note said "besides Ram at the end of the 401k" — could not resolve "Ram." Possibly a name, or "run." Ask before it goes in any client-facing doc.


## 2026-09-15 — HE'S IN
Stephen texted: "Spoke to all my clients, so looks like I am ready to do." Asked whether to target the 9/30 payroll through ADP.
Eli's call: **don't rush 9/30. Start fresh for Q4 (Oct 1).** 2x confirmed through October so there's no forcing function. Sequence: Paychex 401(k)s first → get everything in order → Q4 start.
Next: Thursday sit-down. Bring pay frequencies for each client — weekly payers make an Oct 1 start tight.


## 2026-09-17 — Meeting 1PM (Stephen + McKenzie) — PLAN

### October comp question
Asked Patrick AND the VP whether October pays 100% vs the current 70%. **Nobody knows yet.** October starts next Friday (10/2), so an answer should come soon. Not blocking on it.

### Sequence (what Stephen hears)
1. **401(k)s TODAY** — the piece with a clock. 12-months-free promo ends **9/24**.
   - Get the Paychex-conversion 401(k)s signed off (Walter's Mirrors, Spencer Thwaytes, Onchain Strategies, Upstate Logistics?)
   - Get the NEW 401(k)s signed off (incoming clients who have or want plans)
2. **Grid agreement** — sign **last week of September (9/25–9/30)**, BEFORE the 10/1 Paychex start. (Eli set this 9/17; the email to Stephen says 25th–30th. Earlier draft said first week of October — superseded.)
3. **New-client introductions** — HOLD until the Grid agreement is signed. That's what gets him Platinum 1x on those instead of standard rev share. In his interest.

### THE ONLY THINGS I NEED FROM STEPHEN
- **Bank statements from all the clients** (or voided check / banking screenshot)
- **Emails and phone numbers** for the people on the 401(k)s
- **Balances** — I can log into his Paychex portal and pull these myself (screen share, don't take credentials)
- **WAIT FOR THE LAST PAYCHEX PAYROLLS — the ones due 9/30.** Final YTD registers don't exist until those run. Prior-wage loading needs the FINAL numbers, so the pull happens AFTER 9/30, not before. (Pull a preliminary now so ADP isn't starting from zero; treat the post-9/30 pull as the delta.)

### What I pull from his Paychex portal myself
All the billing / balances / plan information necessary. Screen share — he drives, I direct.

### Outline to build WITH him today — two lists
**A. Clients I set up and sign off WITH Stephen** (he's the point of contact, I work through him)
**B. Clients I send out to directly and work with myself** (I sign off with the client)
Decide client-by-client at the meeting so nobody's confused about who calls whom.

### After today
- [ ] 401(k) submissions in before 9/24
- [ ] Two-list outline (with Stephen vs. direct) written down and shared with him
- [ ] Bank proof + contact info collected per client
- [ ] Grid agreement signing — 9/25–9/30, before first ADP payroll
- [ ] Then: Stephen sends the new-client introductions


## Data audit 2026-09-17 — corrections to my own earlier reads
- **$60/EE/month: TRUE** (see above).
- **Upstate $0.91 (O4): NOT confirmed typo.** N4 (401k flag) is blank but a fee is present. Pull from portal on screen share; don't write off.
- **CLIENT COUNT MISMATCH:** vault history says 19→22 on Paychex; sheet shows 12 Existing. Ask Stephen directly — either the sheet is incomplete or the earlier count was wrong. $18K vs $30K+ book.
- **Column O (401k fees $2,953.16) and P (other CY fees $1,100) are NOT in the M29 total.** True annualized Paychex billing incl. those = $26,499.98. My "excluded by design" was an assumption — Q header only excludes year-end/W2 fees, and O isn't that. Decide the basis in writing before any number goes to Stephen.
- **M29 = SUM(M3:M26) only.** Column A is numbered to 82. Add client #25+ and it silently drops out of the total. Widen the range if the list grows.
- **Kranz Motorcars is the outlier and the constraint:** $3,094/yr = 13.8% of the book, 1 EE, and the ONLY weekly payer. ADP will reprice it; it's the client that makes Oct 1 tight.
- **FL is the largest state footprint (5), not NY (4).** 9 states total.
- **FRG Advisory is New/Gusto** — a competitor conversion, not greenfield. Confirm whether a Gusto conversion is Grid-eligible or falls to Platinum/rev share.
- **Atlas Opera** is the only New client with no 401(k) flag.
- Hygiene: 10 non-breaking spaces (3 between state and ZIP in col H — will break registration imports), 6 trailing spaces, G5 "Steeer" typo, G10 "1625 Cody, Ave", J2/K2 both labeled "Payroll Frequency."
- 2026-09-17: Eli confirmed ALL 12 Existing (Paychex) clients have 401(k) balances → 12 conversions to McKenzie by 9/24, not 6. CSV k401 column set to convert for all Existing.
- 2026-09-17 (correction): NOT 12 conversions. Existing 401(k)s = 3 (Walters, Onchain, Spencer Thwaytes) + Upstate to verify. "Yes" flag = NEW 401(k): 7 (B333, Complete Merchant, AJG, FRG, Kapstone, Broadway 2000, North Star). Need owner name/email/phone for the 7 new.
- 2026-09-17: Eli — do NOT pre-split the 18. Find out from Stephen which clients Eli handles directly vs. which they run together. setup_with reset to "ask".
- 2026-09-20: Walters Mirrors Paychex package (2 payrolls, 9/2 + 9/16) analyzed. BI-WEEKLY (not semi-monthly). Only invoice line: "Paychex HR Pro (Active Employees)" qty 3, $896.70 less $394.55 discount = $502.15 per payroll → $13,055.90/yr. Sheet's $95.30/run was wrong by 5x. 401(k) ($1,201.18/payroll) and WC ($239.76) are remittances, not fees. Research (2 Opus agents, checked): Paychex 408(b)(2) defines "Retirement Services Included Clients" whose 401(k) admin + per-participant fees are INSIDE the bundle fee; HR Pro is marketed as "HR Guidance, Payroll, Safety, 401(k)". So NOT certain the $502.15 excludes 401(k) admin. Annual Account Fee (asset-based) is charged regardless, usually from plan assets. To settle: get Walters' Paychex service agreement fee schedule + 408(b)(2) + 12 months of Paychex bank debits.
- 2026-09-20: Walters Mirrors Q2 2026 Paychex retirement report (22 pp). Plan #651186 "401(k) Profit Sharing Plan and Trust", trustee Niccolo Russino (718) 366-0777. Plan is NEW: $0 at 4/1/26, eligibility date 5/6/26 for all 3, $3,553.92 at 6/30, 100% participation (5%/4%/8%), 100% vested, employee pre-tax only through 6/30 — employer 3% started Q3 (payroll YTD ER = QTD Q3 = $2,324.61). Asset-based fees trivial ($1.39/qtr). Trust statement footnote: "This does not include administration or annual account fees paid from your bank account. Refer to your Invoice." → admin fees exist and are invoiced, but NOT on the two payroll invoices → either a separate retirement invoice Stephen hasn't sent, or bundled in HR Pro. Ask Stephen for any Paychex Retirement Services invoice / bank debit. Watch for early-termination fee on a plan opened May 2026. ADP/ACP test in compliance, no HCEs.
- 2026-09-21: Paychex HRS Billing Statements (Retirement Services, West Henrietta NY) for ONCHAIN (8/4, 9/1) and SPENCER THWAYTES (9/1) — NOT Walters. Pattern per plan: $25 3(16) admin + $150 monthly recordkeeping + $6/participant + **$111.09/mo "SET UP" installment on a $3,999 setup fee (36 months)**; annual account fee ~70 bps from plan assets. Onchain: $292.09/mo ACH = $3,505/yr on a $456 plan. Spencer: $286.09/mo, setup drawn from a second bank account. This is SEPARATE from the payroll invoice → settles the Walters question in practice: expect a third stream, ~$304/mo. Conversion risk: remaining setup installments (~33 of 36) may accelerate on termination — read the Retirement Services Agreement before McKenzie moves any of the three.
- 2026-09-21 (later): WALTERS HRS statements 7/5, 8/5, 9/1 received → separate 401(k) billing CONFIRMED, question closed. $193/mo base ($150 + 3×$6 + $25 3(16)) + setup $83.31/mo on $2,999/36 (Jul, Aug; $249.93 paid) — setup line gone in Sept, unexplained (not paid off). Plan doc fee $30 discounted to $0 in Aug. Walters all-in Paychex ≈ $16.4K/yr for 3 employees. Onchain/Spencer setup is $3,999; Walters $2,999. Spencer's Aug setup installment shows $0 received in both Aug and Sept → likely failed ACH on the second account.
- 2026-09-21 (verification loop, 2 Opus agents adversarial + Fable ruling): CLAIM HOLDS — HR Pro $502.15/payroll and HRS $193/mo are different services, different Paychex units (Paychex of NY LLC Orlando vs Human Resource Services West Henrietta 1-800-472-0072), different cycles, different dates, both actually collected (setup "PAYMENT RECEIVED" counter advances 166.62→249.93). 401(k) is NOT in the payroll charge. Wording fixes: (1) don't argue client-number prefixes — Walters' HRS number changed 0026→0903 in Sept (same as payroll); (2) say "no 401(k) FEE on the payroll invoice" — the $1,201.18 401(k) EFT on the cash page is contributions; (3) there are THREE streams incl. ~70 bps from assets. Sept setup line stopped at installment 4 of 36 — coincides with the branch-number change; unexplained. Reddit r/Payroll: Paychex demanded $1,500 termination + $1,400 remaining setup on cancel → assume acceleration until the Client Fee Disclosure – Additional Services says otherwise. Drop any "double billing" pitch; it isn't supported.
- 2026-09-21: Eli confirmed 401(k) does NOT count toward Grid. Stephen comp: $750 per Paychex-converted plan, $300 per new plan. Walters = Grid 2x on payroll ($6,810.70 ADP annualized → $13,621.40) + $750. ADP 401(k) client price: $300/mo list, McKenzie can go to $175.
- 2026-09-21: ELEVEN Paychex payroll invoices analysed across 6 clients (2 agents + checker + own arithmetic; every invoice reconciles to its printed total). Method per Eli: annual = recurring per-run x runs/yr from Stephen's sheet; exclude one-time setup/penalty; include anything charged every run.
  VERIFIED TIES: Wisdom by Woods 129.78 x12 = 1,557.36 (Delivery $19.50 recurs 2/2, so Stephen's number is right); Spencer Thwaytes 77.00 x12 = 924.00; Backfin 98.50 x24 = 2,364.00 (semi-monthly CONFIRMED by check dates 8/14, 8/31, 9/15); Walters 502.15 x26 = 13,055.90.
  ONLY RATE ERROR: Onchain sheet 58.00 vs invoice 65.00 Officer Only Payroll = +$84/yr.
  ONE-TIME FEES EXCLUDED: $200 Flex Select setup (Wisdom June, Auction July); $50 Missing Local ID (Backfin 8/12 only — gone by 8/26, so cured); $50 Missing State ID (Wisdom + Spencer Aug, status unknown — FLAG).
  BIGGER THAN PRICING — MISSED PAYROLLS: Wisdom's 8/27 invoice has NO payroll line, only the $50 fee. Auction Time Pieces' 8/27 invoice is "Minimum Monthly Billing $65.00" — proof no payroll ran. Both sheets assume every scheduled run happens. If they don't, the Paychex annual is overstated and so is any comparison built on it.
  DELIVERY: recurring for Wisdom ($19.50 on both payroll invoices); NOT constant for Onchain ($36 cannot have been on the prior $85 invoice: 65+36=101>85, residual exactly $20).
  AUCTION UNRESOLVED: July discount -178.96 is sized to absorb the $200 setup (79.10+75.00-178.96 = -24.86), so no clean recurring base derivable from one payroll invoice. Need a normal-month Auction invoice.
- 2026-09-21: Eli: Paychex Direct Deposit Premium Processing is charged every run. Auction Time Pieces recurring = $154.10/run x 24 = $3,698.40, sheet said $1,898.40 (+$1,800). Six verified clients now $22,379.66 vs sheet $20,495.66 (+$1,884).
- 2026-09-21: SCARE, factor into Grid math: Stephen says some accounts on his Paychex roster have NEVER processed a payroll. Consistent with Auction (Minimum Monthly Billing month) and Wisdom (no-payroll month). Grid pays on ADP revenue from accounts that actually run — an account with no payroll history has no run rate to multiply and may not count as a conversion. Eli thinks it's fine but the 6 unverified (Kranz, Upstate, B333, Tyree, Complete Merchant, RJC) are exactly the ones to check for processing history before counting them.
- 2026-09-21: Eli's call — accounts with no Paychex payroll history WILL be submitted toward the Grid as conversions. Risk: if ADP treats them as new (no prior revenue) they fall to Platinum/rev share, not 2x. Don't volunteer the distinction to Stephen; don't build the payout number on them until the Grid desk accepts them.
- 2026-09-22 (Stephen email, per-client status):
  - Kranz: needs client to grant Partner Pro access; reminder sent 9/21.
  - Upstate + Tyree: access granted 9/21, reports pending ~1 day, invoices expected 9/22.
  - B333: set up in Paychex, NEVER ran payroll. Moving straight to ADP without a Paychex run. → no Paychex revenue; Grid eligibility must be confirmed with the desk.
  - Complete Merchant: NOT Paychex — on Gusto, 2 EE, $100/mo, payroll only. Coming to ADP. Not a Grid conversion (Gusto eligibility unverified, same bucket as FRG).
  - RJC Holdings: prior ADP client (>1 yr ago), Stephen moved them to Paychex; Paychex can't link Partner Pro. No invoice. WIN-BACK — check whether a former ADP account counts toward Grid.
  - Broadway 2000: no provider — pays via IRS Treasury online, cash payroll, ~$400/yr. Coming to ADP, quarterly or annual. Prior-year payroll was over-inflated vs distributions; fix on setup. Not a Paychex conversion.
  - FRG Advisory: Gusto, monthly, $50/mo (sheet said $70).
  - Everything else on the sheet = brand-new payroll clients.
  NET: true Paychex conversions with revenue history = Wisdom, Upstate, Auction, Tyree, Onchain, RJC(?), Backfin, Walters, Spencer, Kranz = 10, minus B333 (never ran). Complete Merchant is Gusto, not Paychex.
- 2026-09-22 CORRECTION: outstanding setup balances were overstated as paid. 'PAYMENT RECEIVED' on the HRS statement is CUMULATIVE received, not installments billed. Correct: Walters $2,749.07 (rcvd $249.93 of $2,999), Onchain $3,887.91 (rcvd $111.09 of $3,999), Spencer $3,999.00 (rcvd $0). Total outstanding $10,635.98, not $10,441.58.
- 2026-09-24: UPSTATE LOGISTICS = the Ohio client from Stephen's 9/22 question (EIN 33-4154077 matches). Multi-state: AZ, MI, NJ, OH. ADP agency screens show Taxpay ACTIVE for AZ Revenue, MI Treasury, NJ Treasury + NJ Labor. NOT STARTED for MI Labor & Economic Growth (UIA), OH Job & Family Services, OH Taxation. MISSING ID for Arizona Dept of Economic Security (AZ unemployment). MICHIGAN UIA: employer must authorize ADP as TPA in MiUI (Employer Home → Associated TPAs → TPA Authorization → New → enter ADP's TPA ID). Michigan rule: TPA services BEGIN DATE must be the first day of a calendar quarter and END DATE the last day of a quarter; two TPAs cannot hold Edit or Submit on the same role at once. So Paychex must end 9/30/26 and ADP begin 10/1/26. ADP's Michigan TPA ID must come from implementation — the 50000018 on Stephen's screen is unverified, do NOT assume it's ADP's.
- 2026-09-24 CORRECTION: the Upstate agency/Taxpay screens are PAYCHEX's portal, not ADP's ("Taxpay" is a Paychex trademark). Reading flips: PAYCHEX has AZ Revenue, MI Treasury, NJ Treasury + NJ Labor active since July, and has NEVER set up MI Labor & Economic Growth (unemployment), OH Job & Family Services, OH Taxation, or AZ Dept of Economic Security. That is why the client was told to self-remit Ohio tax — Paychex never got authorized. Paychex's own banner says the gaps cause fees, matching the $50 missing-ID fees on Wisdom, Spencer and Backfin invoices. For a Q4 ADP start: employer authorizes ADP as TPA in MiUI with begin date 10/1/26; Paychex's TPA ends 9/30/26; Paychex still owes Q3 filings due 10/31 on the agencies they actually had set up.
